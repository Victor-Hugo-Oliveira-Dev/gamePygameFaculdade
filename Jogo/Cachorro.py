...
class Cachorro(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        # Comportamento de inércia (adicionando os atributos faltantes)
        self.idle_time = 0
        self.idle_cooldown = random.randint(1000, 3000)
        self.last_idle_change = pygame.time.get_ticks()  # Atributo que estava faltando
    ...
    def die(self):
        ...
        if hasattr(self.game, 'aventureiro'):
            self.game.aventureiro.restore_health(100)  # Restaura 20 pontos de vida
        
    def check_collision(self, new_x, new_y):
        if not self.collision_map:
            return False  # se ainda não foi setado
    ...
    def draw_health_bar(self, surface, camera_x, camera_y):
        """Desenha a barra de vida (não desenha se estiver morrendo)"""
        if self.is_dead or self.alpha < 50:  # Não mostra barra se quase invisível
            return
